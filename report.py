...
class Report:
    ...
    def excel(selfies):
            ...
            ws1.append(['Год', 'Средняя зарплата', 'Средняя зарплата - ' + selfies.names_vacancy, 'Количество вакансий',
                        'Количество вакансий - ' + selfies.names_vacancy])
            selfies.func13(ws1)

            datas = [
                ['Год ', 'Средняя зарплата ', ' Средняя зарплата - ' + selfies.names_vacancy, ' Количество вакансий',
                 ' Количество вакансий - ' + selfies.names_vacancy]]
            columnWth = []
    ...
    def pdf(selfies):
        envar = Environment(loader=FileSystemLoader('../templates'))
        templates = envar.get_template("pdf_template.html")
        stats = []
        selfies.fucn20(stats)

        selfies.fucn21()

        pdfTemplate = templates.render({'name': selfies.names_vacancy, 'path': '{0}/{1}'.format(pathlib.Path(__file__).parent.resolve(), 'graph.png'), 'stats': stats, 'statist5': selfies.statist5, 'statist6': selfies.statist6})

        # config = pdfkit.configuration(wkhtmltopdf=r'/usr/bin/wkhtmltopdf')
        # pdfkit.from_string(pdf_template, 'report.pdf', configuration=config, options={"enable-local-file-access": ""})
        pdfkit.from_string(pdfTemplate, 'report.pdf', options={"enable-local-file-access": ""})

    def fucn21(selfies):
        for keys in selfies.statist6:
            selfies.statist6[keys] = round(selfies.statist6[keys] * 100, 2)

    def fucn20(selfies, stats):
        for years in selfies.statist1.keys():
            stats.append([years, selfies.statist1[years], selfies.statist2[years], selfies.statist3[years],
                          selfies.statist4[years]])
